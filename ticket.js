...
router.get("/:sid/:bid", async (req, res) => {
  var mQuery =
    "select * from Movie where m_id in(select m_id from Shows where s_id='" +
    req.params.sid +
    "')";
  var tQuery =
    "select * from Theater where t_id in(select t_id from Shows where s_id='" +
    req.params.sid +
    "')";
  var sQuery = "select s_time from Shows where s_id='" + req.params.sid + "'";
  var bQuery =
    "select b_status,price,seat_no from Book where b_id='" +
    req.params.bid +
    "'";
